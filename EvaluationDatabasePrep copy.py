...
class dbMaker():
    ...
    tempTopDir = os.path.abspath('/Volumes/Britt_SSD/Singularity2/Evaluations/PacmanStandard')
    ...
    def pullCSVs(self):
        ...
        main_file = os.path.join(self.tempTopDir, 'CSVs/df1main.csv')
        q_file = os.path.join(self.tempTopDir, 'CSVs/df3q_values_df.csv')
        obs_file = os.path.join(self.tempTopDir, 'CSVs/df4obs_df.csv')
        arg_file = os.path.join(self.tempTopDir, 'CSVs/df5argmax_df.csv')
    ...
    def addEpochScores(self):
        ...
        scores_path = os.path.join(self.tempTopDir, 'CSVs/scores.txt')
        ...
        key_states = np.load(os.path.join(self.tempTopDir, 'CSVs/summary_states.npy')).tolist()
        temp_context_states = np.load(os.path.join(self.tempTopDir, 'CSVs/summary_states_with_context.npy'))
        print(key_states)
...
#        print(self.complete_df.to_markdown())
    ...
    def checkImport(self):
        import_temp = pd.read_csv(os.path.join(self.tempTopDir, 'CSVs/state_features_impoartance.csv'), index_col=0)
...
#        print(self.complete_df.head(5).to_markdown())
    ...
    def saveDF(self):
        ...
        pq.write_table(table, 'StandardEvalDB.parquet', compression='BROTLI')
        # And save to CSV for permanent storage
        self.complete_df.to_csv('StandardEvalDB.csv')
        return
        
    def testPQ(self):
        temp_test = pd.read_parquet('StandardEvalDB.parquet')
